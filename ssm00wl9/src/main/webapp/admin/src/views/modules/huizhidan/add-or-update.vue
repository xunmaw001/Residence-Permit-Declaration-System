          ...
          <el-form-item class="input" label="工号" prop="gonghao">
              <el-input v-model="ruleForm.gonghao" 
                placeholder="工号" readonly></el-input>
                                                                  ...
                                                                  <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="回执时间" prop="huizhishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.huizhishijian" 
                type="datetime"
                placeholder="回执时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.huizhishijian" label="回执时间" prop="huizhishijian">
              <el-input v-model="ruleForm.huizhishijian" 
                placeholder="回执时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                                            </el-row>
                                                                                                                          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="回执内容" prop="huizhineirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="回执内容"
                  v-model="ruleForm.huizhineirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.huizhineirong" label="回执内容" prop="huizhineirong">
                    <span>{{ruleForm.huizhineirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `huizhidan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huizhidanCrossAddOrUpdateFlag = false;
                  this.parent.search();
